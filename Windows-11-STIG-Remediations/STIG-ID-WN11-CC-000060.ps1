<#
.SYNOPSIS
This PowerShell script prohibits connections to non-domain networks when a Windows 11 system is connected to a domain-authenticated network.
...
Example syntax:
PS C:\> .\STIG-ID-WN11-CC-000060.ps1
#>
...
if ($currentValue -eq $valueData) {
    Write-Host "COMPLIANT: WN11-CC-000060"
    Write-Host "Connections to non-domain networks are blocked while connected to a domain-authenticated network."
} else {
    Write-Host "NON-COMPLIANT: WN11-CC-000060"
