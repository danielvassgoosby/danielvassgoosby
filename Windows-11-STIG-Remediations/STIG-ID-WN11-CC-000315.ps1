<#
.SYNOPSIS
This PowerShell script disables the Windows Installer "Always install with elevated privileges" policy.
...
if ($null -ne $currentValue -and $currentValue -eq $valueData) {
    Write-Host "COMPLIANT: WN11-CC-000315"
    Write-Host "AlwaysInstallElevated is disabled."
} else {
    Write-Host "NON-COMPLIANT: WN11-CC-000315"
